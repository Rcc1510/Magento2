...
*/
namespace Kitchen\Notification\Ui\Component\Listing\Column;
...
*/
class NoticeActions extends Column
{
    /** Url path */
    // 'admin_route_name/folder_name/file_name'
    const CMS_URL_PATH_EDIT = 'notice1/index/create';
    const CMS_URL_PATH_DELETE = 'notice1/index/delete';
        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item['notification_id'])) {
                    $item[$name]['edit'] = [
                        'href' => $this->urlBuilder->getUrl($this->editUrl, ['notification_id' => $item['notification_id']]),
                        'label' => __('Edit'),
                    ];
                    $title = $this->getEscaper()->escapeHtml($item['title']);
                    $item[$name]['delete'] = [
                        'href' => $this->urlBuilder->getUrl(self::CMS_URL_PATH_DELETE, ['notification_id' => $item['notification_id']]),
                        'label' => __('Delete'),
                        'confirm' => [
